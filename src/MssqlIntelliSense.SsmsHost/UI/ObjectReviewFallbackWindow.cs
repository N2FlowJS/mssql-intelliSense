...
internal sealed class ObjectReviewFallbackWindow : Window
{
    private static ObjectReviewFallbackWindow? _instance;
    private readonly ObjectReviewWindow _panel;

    private ObjectReviewFallbackWindow()
    {
    ...
    public static void ShowForCompletion(SqlCompletionItem item, DatabaseMetadata metadata, Window? owner)
    {
        ...
        var window = _instance ??= new ObjectReviewFallbackWindow();
        if (owner != null)
        {
            window.Owner = owner;
...
{
    public static System.Windows.Media.Brush GetBrush(object key, System.Windows.Media.Color fallback)
    {
